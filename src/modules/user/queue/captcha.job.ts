...
export class CaptchaJob {
    ...
    async send(params: SendParams): Promise<{ result: boolean; log: any }> {
        ...
        try {
            // 获取验证码发送配置
            const time = getUserConfig<CaptchaTimeOption | undefined>(`captcha.time.${action}`);
            const config = getUserConfig<CaptchaTimeOption | undefined>(`captcha.email.${action}`);
            ...
            const expired = getUserConfig<number>(`captcha.time.${action}.expired`);
            const otherVars =
